    ...
    <footer className="relative mt-32 border-t border-line bg-bg2">
      <div className="mx-auto grid max-w-shell gap-12 px-5 py-20 lg:grid-cols-4 lg:px-10">
        <div className="lg:col-span-2">
          <div className="font-display text-3xl font-semibold uppercase leading-[0.95] tracking-tight">
            Arizona Home <span className="text-accent">&amp; Landscape</span>
          </div>
          ...
          <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-line px-4 py-2 text-sm">
            ...
            <span className="text-muted">· {site.reviewCount} Google reviews</span>
          </div>
        </div>
        <div>
          ...
          <ul className="mt-5 space-y-3 text-sm">
            ...
            <li><Link href="/contact" className="link-sweep">Contact</Link></li>
          </ul>
        </div>
        <div>
          ...
          <address className="mt-5 space-y-3 text-sm not-italic text-fg/90">
            ...
            <div className="flex items-start gap-2.5"><MapPin size={16} className="mt-0.5 text-accent" />{site.street}, {site.city}, {site.region} {site.zip}</div>
            <div className="flex items-start gap-2.5"><Clock size={16} className="mt-0.5 text-accent" />Mon–Fri 8am–4pm · Sat–Sun closed</div>
          </address>
        </div>
      </div>
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-shell flex-col gap-2 px-5 py-6 text-xs text-muted sm:flex-row sm:items-center sm:justify-between lg:px-10">
          <div>© {new Date().getFullYear()} {site.name}. Family-owned in Chandler since {site.founded}.</div>
